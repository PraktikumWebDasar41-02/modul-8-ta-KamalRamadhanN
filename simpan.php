...
class oop
{
	...
	include 'koneksi.php';
		...
		if (isset($this->user)&&isset($this->pass)&&isset($this->email)) {
			$this->connn = $conn;
			$this->qry = mysqli_query($this->connn,"INSERT INTO user(id, name, email, pass) VALUES ('','$this->user','$this->email','$this->pass')");
			if (!$this->qry) {
				die("GAGAL");
			}else{
				echo "Berhasil";
	...
	function tambahnew($namad,$namab,$nim,$kelas,$hobi,$genre,$tw)
	{
		...
		if (isset($this->ni)&&isset($this->nd)&&isset($this->nb)) {
			$this->connn = $conn;
			$this->qry = mysqli_query($this->connn,"INSERT INTO data(nama_depan, nama_belakang, Nim, kelas, hobi, genre, tempat, id)
	...
	function hapus($d)
	{
		...
		$this->id2 = $d;
		$this->connn = $conn;
			$this->qry = mysqli_query($this->connn,"DELETE FROM data WHERE Nim ='$this->id2'");
			if (!$this->qry) {
				die("GAGAL");
			}else{
				echo "Berhasil";
